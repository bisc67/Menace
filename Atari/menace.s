...
MODE_SEEK           equ 1
MODE_????           equ 2
MODE_UPDOWNANIM     equ 3
MODE_ANIMATEDOWN    equ 4
MODE_HEATSEEKER     equ 5
MODE_SEEK_ON_X      equ 6
MODE_SEEK_ON_Y      equ 7
...
MainMenu
            ...
            btst        #$7,joy.btn(a6)
            bne.s       start.game2
            bsr         +$27d2		// 2ec8 - show intro bitmap?
            bra.s       start.game2
enter.game
			bsr			init.screen2
			bsr			draw.titles
...
@noguardian	
            ...
            bmi         +$2076		// $2832
            clr.l       d1
            move.w      ship.frame(a6),d1
            move.w      ship.x(a6),d2
            move.w      ship.y(a6),d3
            cmp.w       #$10,d1
            bcc.s       @noupdate
            addi.w      #$1,ship.frame(a6)
            andi.w      #$7,d1
            moveq       #0,d0			// draw.ship or explosion or bonus?
            bsr		    draw.bob
            bra.s       @noupdate
@noexplode
			bsr			check.fire
			bsr			update.position
@noupdate	bsr			draw.ship
			bsr			draw.aliens
			bsr			check.collision
			bsr 		draw.outrider
...
write.palette moveq       #7,d3
@loop       moveq       #SYS_SET_AUDIO,d0
            trap        #4
            dbra        d3,@loop
            rts 
reset.globals
...
@loop
			...
			moveq		#3,d2
			bsr			draw.bar
			lea			LevelPalettes,a0
			move.w		level.number(a6),d0
			mulu		#$60,d0
			add.l		a0,d0
			move.l		d0,LevelPalettePtr(a6)
0009cc : 42ae 00d6                : clr.l player.score(a6)
			clr.w		no.bonus(a6)
...
0009f0 : 51ee 0112                : sf  demo.mode(a6)
...
@noclip		mulu		#$60,d0
			adda.l		d0,a2
			moveq		#15,d0
@blocklp	move.w		(a2)+,(a0)+                     // bitplane 0
			move.w		(a1)+,(a0)+                     // bitplane 1
			move.w		30(a2),(a0)+                    // bitplane 0, 16 pixels to right
			move.w		30(a1),(a0)+                    // bitplane 1, 16 pixels to right
			move.w		62(a2),(a0)+                    // bitplane 0, 32 pixels to right
			move.w		62(a1),(a0)+                    // bitplane 1, 32 pixels to right
			dbra		d0,@blocklp
			dbra		d1,@linelp
			moveq		#0,d1
...
000ade : 247c 0007 b100           : movea.l #GFX_GUARDIAN,a2
...
//
// Image is 256 x 192 pixels wide. Processed in 16 strips, 24 blocks high, each block 16x8 pixels
//
draw.guardian
			movea.l		screen.base(a6),a1
			cmpa.l		#SCREEN_BASE,a1
			beq.s		@done
			movea.l		#GFX_GUARDIAN,a2						// A2 - base of guardian data. a bitfield used to compress
			lea			384(a2),a3				
			move.w		#BYTES_PER_LINE-8,d3					// Number of lines
			moveq		#16-1,d7
@lineloop	movea.l		a1,a0
			moveq		#24-1,d6								
@nextbyte	move.b		(a2)+,d4								// Read compress bitfield
			moveq		#(128/16)-1,d5
@nextbit	btst		d5,d4									// Is there a non zero 16 pixel block?
			bne.s		@nonzero
			clr.l		(a0)+									// Clear bitplanes 0,1
			clr.l		(a0)+									// Clear bitplanes 2,3
			bra.s		@endloop
@nonzero	move.l		(a3)+,(a0)+								// Get bitplanes 0,1
			move.w		(a3)+,(a0)+								// bitplane 2
			clr.w		(a0)+									// nothing in bitplane 3
@endloop	adda.w		d3,a0									// Add line modulo
			dbra		d5,@nextbit
			dbra		d6,@nextbyte
			addq.w		#8,a1									// On to next screen location (why this? we don't need this
																/ or the instruction it branches back to. It should branch +1 instr
			dbra		d7,@lineloop
			rts 
@done
			move.w		#$4,game.stage(a6)
			rts 
			// D0 - Height?
			// D2 - x coordinate
			// D3 - y coordinate
			...
			// 
draw.bob
			move.w		d2,d5									// This is some sort of sprite rendering code
			subq.w		#2,d2									// Is it off screen? I cheated here by offsetting the x position by 2 pixels to aide easier clipping. 
			bls			return									// Pretty crap if you asked me nowadays. May have shaved one or two clock cycles here and there
			movea.l		screen.base(a6),a0
			mulu		#BYTES_PER_LINE,d3
			adda.w		d3,a0									// Add Y offset to screen address
			move.l		d2,d3									// Copy X coord
			andi.w		#$f,d2									// Make it in to a bit offset within a word
			lsr.w		#1,d3									// Make copy in to a byte offset
			andi.w		#$f8,d3									// Get rid of the bit position. I did this in 16 bit chunks.
			adda.w		d3,a0
			suba.w		#$10,a0
			move.l		d5,d6
			// Create a mask for the left and the right side of the sprite. This allows it
			// to get clipped properly when going off screen. I'm *SURE* this could be written
			// much more efficiently.
			moveq		#-1,d7
			subi.w		#$20,d5
			bhi.s		@notleftclipped
			move.w		d6,d5
			subq.w		#2,d5
			bls.s		@norightmask
			neg.w		d5
			andi.w		#$1f,d5									// Make x coord to a bit offset within a long
			lsr.l		d5,d7									// Shift mask in to correct place
			bra.s		@jump4
@notleftclipped
			subi.w		#224+2,d5								// Is it clipped to the right?
			bcs.s		@notrightclipped
			cmp.w		#32,d5									// if it's within the last 32 pixels, shift the mask accordingly
			bls.s		@rightmask
@norightmask
			clr.l d7
			bra.s		@notrightclipped
@rightmask
			lsl.l		d5,d7
@notrightclipped
			move.l		d7,edge.mask(a6)
			bclr		#$7,d0
			bne			@upsidedown								// Our bob is flipped on y
			cmp.w		#$1,d0
000cda : 6612                     : bne.s @jump1
...
000cec : 602c                     : bra.s @jump2
@jump1
000cee : b07c 0002                : cmp.w #$2,d0
000cf2 : 6612                     : bne.s @jump3
...
process.loop
			...
			beq			same.anim
			btst		#MODE_UPDOWNANIM,mode(a0)
			beq			up.only
			btst		#MODE_ANIMATEDOWN,mode(a0)
			bne			down.anim
			addq.b		#1,anim.num(a0)							// increase anim num
			cmp.b		anim.num(a0),d3
			bne			process1
			bchg		#MODE_ANIMATEDOWN,mode(a0)
			bra			process1
down.anim
...
no.pause
			...
			btst		#MODE_OFFSET,mode(a0)					// test for offset mode
			bne			add.offsets
...
store.coords
			...
			btst		#MODE_HEATSEEKER,mode(a0)				// heat seeker ?
			beq			update
			bsr			explode.alien							// heat seekers explode when they
			bra			finished								// hit their target
update
			btst		#MODE_SEEK,mode(a0)						// check for seek mode
			beq			new.offset
			bsr			copy.coords
			subq.b		#1,seek.count(a0)
			bne			finished
			bclr		#MODE_SEEK,mode(a0)						// reset seek mode
...
check.seek
			btst		#MODE_SEEK_ON_X,mode(a0)   				// bit 6 for seek on ship x
			beq			check.seeky
			...
			bne			check.seeky
			bclr		#MODE_SEEK_ON_X,mode(a6)				// reset if count zero
check.seeky
			btst		#MODE_SEEK_ON_Y,mode(a6)				// bit 7 for seek on ship y
			beq			check.heat
			...
			subq.b		#1,seek.count(a0)						// check count
			bne			check.heat
			bclr		#MODE_SEEK_ON_Y,mode(a0)	    		// reset if count zero
check.heat
			btst		#MODE_HEATSEEKER,mode(a0)				// bit 5 for a heat seeking mine
...
toggle.offset
			bchg		#MODE_OFFSET,mode(a0)					// start or end offset mode
			bra			update									// get the next path value
				
seek.mode
			bset		#MODE_SEEK,mode(a0)						// set the seek mode bit
...
start.seekx
			bset		#MODE_SEEK_ON_X,mode(a0)				// set the seek on X bit
			move.b		1(a0,d0.w),seek.count(a0)				// copy the count
			addq.w		#2,d0
			move.w		d0,table.offset(a0)
			bra			finished
...
start.seeky
			bset		#MODE_SEEK_ON_Y,mode(a0)				// set the seek on Y bit
			move.b		1(a0,d0.w),seek.count(a0)				// copy the count
			addq.w		#2,d0
			move.w		d0,table.offset(a0)
			bra			finished
...
copy.coords
            btst        #MODE_HEATSEEKER,mode(a0)				// heatseek mode?
            bne         next.stage
...
001348 : 51ee 0098                : sf  +$98(a6)
			...
			bra			copy.path	
check.end
; For some reason, I often used branches back to this point for a 'error return'. I remember thinking it
; was a lame way to deal with it at the time, just never changed it.
return
			rts

start.path  tst.b       +$78(a6)
			bne.s		return
...
00142c : 302e 00c4                : move.w path.index(a6),d0
...
001446 : 3d40 00c4                : move.w d0,path.index(a6)
...
0016e0 : 45f9 0000 4a2c           : lea GFX_PALETTE,a2
0016e6 : 47f9 0000 4a4c           : lea GFX_PALETTE_MIRROR,a3
...
@drawloop	lea			8(a0),a2
			...
			move.w		(a1)+,d5								// Get bitplane 2 data
...
001c66 : 43ee 00d6                : lea player.score(a6),a1
...
001d6e : b06e 0084                : cmp.w ship.frame(a6),d0
001d72 : 6712                     : beq.s +18
001d74 : 7007                     : moveq #7,d0
001d76 : b06e 0084                : cmp.w ship.frame(a6),d0
001d7a : 670a                     : beq.s +10
001d7c : 0c6e 0001 0084           : cmpi.w #$1,ship.frame(a6)
001d82 : 6702                     : beq.s +2
001d84 : 700a                     : moveq #10,d0
001d86 : 3d40 0084                : move.w d0,ship.frame(a6)
...
001e00 : 0c6e 000a 0084           : cmpi.w #$a,ship.frame(a6)
001e06 : 670c                     : beq.s +12
001e08 : 0c6e 0004 0084           : cmpi.w #$4,ship.frame(a6)
001e0e : 6704                     : beq.s +4
001e10 : 566e 0084                : addq.w #3,ship.frame(a6)
...
fade.palette
001f1a : 41f9 0000 4a2c           : lea GFX_PALETTE,a0			// Dest palette for the fade
001f20 : 6100 03f8                : bsr .l do.fade			// 231a
001f24 : 4a00                     : tst.b d0
001f26 : 6624                     : bne.s @done
001f28 : 4a2e 0073                : tst.b +$73(a6)
001f2c : 661a                     : bne.s +26
001f2e : 50ee 0073                : st  +$73(a6)
001f32 : 41f9 0000 4a4c           : lea GFX_PALETTE_MIRROR,a0
001f38 : 43f9 0000 4a2c           : lea GFX_PALETTE,a1
...
001f74 : 3d7c 0001 0084           : move.w #$1,ship.frame(a6)
...
00200c : 4a2e 0112                : tst.b demo.mode(a6)
...
0020d0 : 6100 0134                : bsr .l @alloc.projectile
0020d4 : 6700 014a                : beq .l @done
...
002106 : 6100 00fe                : bsr .l @alloc.projectile
00210a : 6700 0114                : beq .l @done
...
002142 : 6100 00c2                : bsr .l @alloc.projectile
002146 : 6700 00d8                : beq .l @done
...
00217e : 6100 0086                : bsr .l @alloc.projectile
002182 : 6700 009c                : beq .l @done
...
002196 : 6100 006e                : bsr .l @alloc.projectile
00219a : 6700 0084                : beq .l @done
...
Trap4Handler
			...
			cmp.w		#SYS_SET_AUDIO,d0						// have been more efficient
			beq.s		SetAudioRegisters
			cmp.w		#SYS_SET_PALETTE,d0
			beq			SetPaletteRegisters
...
@nowrap		andi.w		#$e0,d0
			...
			rte 
KeyboardHandler
            movem.l     d0-1/a0/a6,-(a7)                        ; Save some registers
            clr.w       d0
            lea         GlobalVariables,a6                      ; Set pointer to globals
            movea.l     #HW_ACIA_BASE,a0                        ; base of keyboard registers
            move.b      (a0),d0                                 ; Get status
0024fe : 0800 0007                : btst #$7,d0
002502 : 6700 0046                : beq .l @done
002506 : 0800 0000                : btst #$0,d0
00250a : 673e                     : beq.s @done
            move.b      2(a0),d0                                ; Key result code
002510 : 4a2e 0066                : tst.b +$66(a6)
002514 : 6652                     : bne.s +82 {$002568}
002516 : 4a2e 0067                : tst.b +$67(a6)
00251a : 665a                     : bne.s +90 {$002576}
00251c : 4a2e 0068                : tst.b +$68(a6)
002520 : 6706                     : beq.s +6 {$002528}
002522 : 422e 0068                : clr.b +$68(a6)
002526 : 6022                     : bra.s +34 {$00254A}
002528 : b07c 00fe                : cmp.w #$fe,d0
00252c : 57ee 0068                : seq +$68(a6)
002530 : b07c 00ff                : cmp.w #$ff,d0
002534 : 57ee 0066                : seq +$66(a6)
002538 : 6710                     : beq.s +16 {$00254A}
00253a : 3200                     : move.w d0,d1
00253c : 0241 00fc                : andi.w #$fc,d1
002540 : b27c 00f8                : cmp.w #$f8,d1
002544 : 6712                     : beq.s +18 {$002558}
            move.b      d0,keycode(a6)                          ; Store keycode
@done       bclr        #$6,$fffffa11                           ; Interrupt ack
            movem.l     (a7)+,d0-1/a0/a6                        ; Restore regs saved earlier
            rte                                                 ; All done!
...
0025d2 : 4a2e 0112                : tst.b demo.mode(a6)
0025d6 : 6606                     : bne.s +6
...
@printmsg
			moveq		#SYS_PRINT,d0
			trap		#4
0026a4 : 41f9 0000 40fc           : lea font.palette,a0
0026aa : 6100 e1b2                : bsr .l write.palette
			move.w		#500,d7
@waitloop
...
002754 : 41f9 0000 40fc           : lea font.palette,a0
00275a : 6100 e102                : bsr .l write.palette
...
00279e
			bsr			disable.hblank
			move.l		#SCREEN_BASE,screen.base(a6)
...
start.demo
002812 : 50ee 0112                : st  demo.mode(a6)
002816 : 3d7c 0012 0110           : move.w #$12,+$110(a6)
00281c : 3d7c 0000 0084           : move.w #$0,ship.frame(a6)
...
002836 : 4e44                     : trap #4
002838 : 51ee 0112                : sf  demo.mode(a6)
...
002848 : 20ae 00d6                : move.l player.score(a6),(a0)
00284c : 6100 ff48                : bsr .l -$b8
002850 : 41f9 0000 4feb           : lea MSG_Dead,a0
002856 : 7006                     : moveq #SYS_PRINT,d0
002858 : 4e44                     : trap #4
00285a : 41f9 0000 40fc           : lea font.palette,a0
002860 : 6100 dffc                : bsr .l write.palette
002864 : 7007                     : moveq #SYS_DELAY_FRAMES,d0
002866 : 724b                     : moveq #75,d1
002868 : 4e44                     : trap #4
00286a : 6100 ff2a                : bsr .l -$d6
00286e : 41f9 0000 532e           : lea MSG_ScoreLabels,a0
002874 : 7006                     : moveq #SYS_PRINT,d0
002876 : 4e44                     : trap #4
002878 : 41f9 0000 5403           : lea ScoreCoordTable,a0
...
0028c6 : 41f9 0000 40fc           : lea font.palette,a0
0028cc : 6100 df90                : bsr .l write.palette
...
0028f8 : 41f9 0000 40fc           : lea font.palette,a0
0028fe : 6100 df5e                : bsr .l write.palette
...
0029d4 : 41f9 0000 541e           : lea MSG_LetterMap,a0
...
0029ec : 41f9 0000 40fc           : lea font.palette,a0
0029f2 : 6100 de6a                : bsr .l write.palette
...
002a42 : 6100 0e08                : bsr .l SaveHighScores
002a46 : 422e 0112                : clr.b demo.mode(a6)
002a4a : 6100 fd4a                : bsr .l -$2b6
002a4e : 41f9 0000 5106           : lea MSG_RestartLevel,a0
002a54 : 7006                     : moveq #SYS_PRINT,d0
002a56 : 4e44                     : trap #4
002a58 : 41f9 0000 40fc           : lea font.palette,a0
...
002c36 : 21ae 00d6 0000           : move.l player.score(a6),0(a0,d0.W)
...
002cae : 41f9 0000 521f           : lea MSG_TryAgain,a0
002cb4 : 7006                     : moveq #SYS_PRINT,d0
002cb6 : 4e44                     : trap #4
002cb8 : 41f9 0000 40fc           : lea font.palette,a0
002cbe : 6100 db9e                : bsr .l write.palette
...
002d70 : 41f9 0000 512b           : lea MSG_MapNames,a0
...
002dec : 41f9 0000 40fc           : lea font.palette,a0
002df2 : 6100 da6a                : bsr .l write.palette
...
002eea : 207c 0001 0000           : movea.l #SCREEN_BASE,a0
002ef0 : 303c 7fff                : move.w #$7fff,d0
002ef4 : 4298                     : clr.l (a0)+
002ef6 : 51c8 fffc                : dbra d0,-4
002efa : 207c 0001 0000           : movea.l #SCREEN_BASE,a0
...
002f1e : 6100 d93e                : bsr .l write.palette
...
003174 : 6100 d6e8                : bsr .l write.palette
...
00318e : 323c 0064                : move.w #100,d1
003192 : 4e44                     : trap #4
...
00320a : 207c 0001 0000           : movea.l #SCREEN_BASE,a0
...
00321e : 4e44                     : trap #4
003220 : 41f9 0000 519a           : lea MSG_Complete,a0
003226 : 7006                     : moveq #SYS_PRINT,d0
003228 : 4e44                     : trap #4
00322a : 41f9 0000 40fc           : lea font.palette,a0
003230 : 6100 d62c                : bsr .l write.palette
...
0036a2 : 4e44                     : trap #4
0036a4 : 41f9 0000 40fc           : lea font.palette,a0
0036aa : 6100 d1b2                : bsr .l write.palette
0036ae : 4e75                     : rts 
play.music
...
00371a : 6604                     : bne.s +4
00371c : 6100 f734                : bsr	-$8cc
003720 : 6100 d124                : bsr	clear.palettes
003724 : 6000 f760                : bra	disable.hblank
003728 : 4e75                     : rts 
...
0037a0 : 4e75                     : rts 
; Check to make sure we have the desired disk in the drive
; D5 - Disk we want
CheckDiskSignature
            move.w      #$a,error.retry(a6)
RetrySignature
            st          $38a
            move.w      +$11e(a6),d1
            cmp.b       #$1,d5
...
.noclear    moveq       #1,d2
            ...
            nop 
            movem.l     (a7)+,d5-7/a6
            tst.w       error.retry(a6)
            bra.s       .exit
            tst.l       d0
            bne         InsertDisk
            lea         +$3e82(a6),a0
            cmpi        #$44524143,(a0)+        ; Signature check
            bne         InsertDisk
            cmpi        #$4f4e4941,(a0)+
            bne         InsertDisk
            move.b      (a0)+,d0                ; Disk ID # check
            subi.b      #$30,d0
            cmp.b       d5,d0
            bne         InsertDisk
.exit       bsr         +$c2    ; 38c4
...
LoadHighScores
            moveq       #2,d5                   ; Make sure we're on disk 2?
            bsr         CheckDiskSignature
            move.w      +$11e(a6),d1
            moveq       #6,d2
            moveq       #1,d3
            moveq       #0,d4
            ...
            jsr         $d4000
            movem.l     (a7)+,d5-7/a6
            lea         +$3e80(a6),a5           ; Disk Buffer?
            lea         MSG_HiScoreSignature,a0 ; Verify high score signature
            moveq       #19,d0
.loop       cmpm.b      (a0)+,(a5)+
            bne.s       .failed
            dbf         d0,.loop
            move.w      #$12b,d0                ; If signature ok, copy the high score table
.copyloop   move.b      (a5)+,(a0)+
            dbra        d0,.copyloop
.failed
            rts 
...
SaveHighScores
            moveq       #2,d5                   ; Make sure we're on disk 2?
            bsr         CheckDiskSignature
            tst.l       d0                      ; Any problems, restart to main loop
            bne         return
            move.w      +$11e(a6),d1
            moveq       #6,d2
            moveq       #1,d3
            moveq       #0,d4
            lea         MSG_HiScoreSignature,a0
            movem.l     d5-7/a6,-(a7)
            moveq       #1,d0
            jsr         $d4000
            movem.l     (a7)+,d5-7/a6
...
00387a : 6100 ff26                : bsr .l CheckDiskSignature
...
0038ac : 41f9 0000 531c           : lea MSG_DiskError,a0
...
InsertDisk
            move.l      screen.base(a6),-(a7)
            clr.l       d0
            move.b      HW_VIDEO_BASE+1,d0
            swap        d0
            move.b      HW_VIDEO_BASE+3,d0
            lsl.w       #8,d0
            move.l      d0,-(a7)
            movea.l     d0,a1
            adda.l      #$8000,a1
            cmpa.l      #SCREEN_BASE+10*$8000,a1
            bcs.s       .nowrap
            movea.l     #SCREEN_BASE,a1
.nowrap     move.l      a1,screen.base(a6)
            move.w      #$1f3f,d0
.clearlp    clr.l       (a1)+
            dbf         d0,.clearlp
            lea         MSG_InsertDiskA,a0
            cmp.b       #$1,d5
            beq.s       .diska
            lea         MSG_InsertDiskB,a0
.diska
            moveq       #SYS_PRINT,d0
            trap        #4
            move.l      screen.base(a6),86(a6)
...
.copylp     move.l      (a0),(a1)+
            ...
            clr.b       joy.btn(a6)
            clr.b       keycode(a6)
.waitlp     moveq       #SYS_SET_VIDEOBASE,d0
            moveq       #1,d1
            trap        #4
            tst.b       keycode(a6)
            beq.s       .waitlp
            move.l      (a7)+,86(a6)
            move.l      (a7)+,screen.base(a6)
            moveq       #SYS_SET_VIDEOBASE,d0
            trap        #4
            movea.l     #HW_VIDEO_PALETTE,a0
            lea         GFX_PALETTE_MIRROR,a1
...
KeyboardInit
		dc.w	$ff80               ; Reset the controller
		dc.w	$ff01               ; Second part of reset controller
		dc.w	$ff08               ; Enable relative mouse positions
		dc.w	$ff0b               ; Set mouse threshold
		dc.w	$ff06               ; x threshold
		dc.w	$ff06               ; y threshold 
		dc.w	$ff07               ; Set position report mode so that
		dc.w	$ff04               ; mouse button presses are keypresses
		dc.w	$0000 
...
MSG_GameOverTotals
			...
			dc.b		$32,$14,"FULL SHIELD POWER",-1
MSG_RestartLevel
			dc.b		$2d,$05,"PRESS FIRE TO",0
			dc.b		$1e,$0b,"RESTART THIS LEVEL",-1
MSG_MapNames
            dc.b        $32,$02,"SEA OF KARNAUGH",0
            dc.b        $32,$06,"VANGUARD WARZONE",0
            dc.b        $32,$0a,"CARNAGE RIFT",0
            dc.b        $32,$0e,"TROPICS OF MACE",0
            dc.b        $32,$12,"RUINS OF KRUGER",0
            dc.b        $32,$16,"PLATEAUS OF DRACONIA",-1
MSG_Complete
            dc.b        $23,$04,"CONGRATULATIONS!",0
            dc.b        $04,$08,"YOU HAVE BRAVELY FOUGHT YOUR",0
            dc.b        $0a,$0a,"WAY THROUGH, AND DESTROYED",0
            dc.b        $30,$0c,"DRACONIA.",0
            dc.b        $05,$10,"NOW RETURN TO THE MOTHERSHIP",0
            dc.b        $39,$12,"AND REST.",-1
MSG_TryAgain
            dc.b        $26,$04,"WHY NOT CONTINUE",0
            dc.b        $08,$08,"THIS TIME THE DRACONIANS WILL",0
            dc.b        $1e,$0a,"BE OUT FOR REVENGE!",0
            dc.b        $32,$0d,"GOOD LUCK",-1
            
5275 :  1e                     : line-F #$f1e
            dc.b        $1e,$06,"TO RESTART THIS LEVEL",0
            dc.b        $32,$10,"PRESS FIRE",-1

529a
            dc.b        $40,$0b,"   ".-1
MSG_InsertDiskA
            dc.b        $28,$07,"INSERT MENACE DISK",0
            dc.b        $2a,$0a,"A INTO DRIVE A:",0
            dc.b        $28,$0d,"THEN PRESS ANY KEY",-1
MSG_InsertDiskB
            dc.b        $28,$07,"INSERT MENACE DISK",0
            dc.b        $2a,$0a,"B INTO DRIVE A: ",0
            dc.b        $28,$0d,"THEN PRESS ANY KEY",-1
            dc.b        0
MSG_DiskError
        dc.b    $50,$00,"DISK READ ERROR",-1
MSG_ScoreLabels
        dc.b    $08,$00,"LEVEL 1 SCORE",0
        dc.b    $08,$02,"LEVEL 2 SCORE",0
        dc.b    $08,$04,"LEVEL 3 SCORE",0
        dc.b    $08,$06,"LEVEL 4 SCORE",0
        dc.b    $08,$08,"LEVEL 5 SCORE",0
        dc.b    $08,$0a,"LEVEL 6 SCORE",0
        dc.b    $2c,$0d,"TOTAL",0
        dc.b    $08,$10,"SPEEDUPS",0
        dc.b    $08,$12,"OUTRIDERS",0
        dc.b    $08,$14,"F FIELDS",0
        dc.b    $56,$10,"CANNONS",0
        dc.b    $56,$12,"LASERS",0
        dc.b    $56,$14,"SHIELDS",-1
ScoreCoordTable
        dc.b    $58,$00
        dc.b    $58,$02
        dc.b    $58,$04
...
MSG_HiScoreSignature
        dc.b    "THE HIGH SCORE TABLE...."
MSG_HighScores
        dc.b    $16,$00,"1 250000",$44,$00,"....BMC WATSON",0
        dc.b    $14,$02,"2 200000",$44,$02,"....DAVE JONES",0
        dc.b    $14,$04,"3 175180",$44,$04,"....MARL      ",0
        dc.b    $14,$06,"4 177040",$44,$06,"....OZZ       ",0
        dc.b    $14,$08,"5 150000",$44,$08,"....TONY SMITH",0
        dc.b    $14,$0a,"6 100000",$44,$0a,"....DMA DESIGN",0
        dc.b    $14,$0c,"7 080000",$44,$0c,"....RUSS--MIKE",0
        dc.b    $14,$0e,"8 060000",$44,$0e,"....ANDY--GOGS",-1
